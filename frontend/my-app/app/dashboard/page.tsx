...
import StatusPill, { type StressLevel } from '../components/StatusPill'

export default function DashboardPage() {
  const router = useRouter()

  // Hold 3-level stress status + averaged ratios for the pill tooltip
  ...
  return (
    <main className="h-screen w-screen bg-black text-white flex flex-col items-center overflow-hidden">
      ...
      <header className="relative w-full max-w-7xl px-4 pt-4 pb-2 flex flex-col items-center">
        ...
        {/* Right-side actions: Go + Status pill */}
        <div className="absolute top-4 right-4 flex items-center gap-2">
		  <StatusPill level={stressLevel} ba={ba} bt={bt} />
          <button
            onClick={() => router.push('/serenity')}
            className="bg-neutral-900 hover:bg-purple-600 text-gray-300 hover:text-white text-xs px-3 py-1 rounded-md border border-neutral-700 transition-colors"
          >
            Go to Serenity →
          </button>
        </div>
      ...
      >
        ...
        <div className="col-span-1 flex">
          <EEGChart
            onStatusChange={(level, { baAvg, btAvg }) => {
              setStressLevel(level)
              setBa(Number.isFinite(baAvg) ? baAvg : 0)
              setBt(Number.isFinite(btAvg) ? btAvg : 0)
            }}
          />
        </div>
        <div className="col-span-1 flex">
